...
class Ion_auth
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $group_name = false) //need to test email activation
	{
		$email_activation = $this->ci->config->item('email_activation', 'ion_auth');

		if (!$email_activation)
		{
			$id = $this->ci->ion_auth_model->register($username, $password, $email, $group_name);
			if ($id !== FALSE)
			{
